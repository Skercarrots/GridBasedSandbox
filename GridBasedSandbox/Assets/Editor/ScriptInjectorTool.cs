...
public class ScriptInjectorDynamic : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.HelpBox("Instructions: Add scripts, drag them into folders, and click Finalize.", MessageType.Info);
    ...
    private void DrawInbox()
    {
        ...
        GUILayout.Label("AVAILABLE SCRIPTS", EditorStyles.miniBoldLabel);
        if (GUILayout.Button("Add from PC", EditorStyles.miniButton, GUILayout.Width(100)))
        {
    ...
    // New parameter: indentLevel to handle spacing manually
    private void DrawHierarchy(FolderNode node, int indentLevel)
    {
        ...
        if (node.isExpanded)
        {
            ...
            for (int i = 0; i < node.scriptsInside.Count; i++)
            {
                ...
                HandleDragStart(new Rect(sRect.x + scriptIndent, sRect.y, sRect.width, sRect.height), node.scriptsInside[i], false, node);
            }
            ...
            for (int i = 0; i < node.subFolders.Count; i++)
            {
                ...
                // Get the last rect drawn (which is the subfolder row) to place the minus button
                Rect lastFolderRect = GUILayoutUtility.GetLastRect(); 
                if (GUI.Button(new Rect(lastFolderRect.x + lastFolderRect.width - 18, lastFolderRect.y, 20, 18), EditorGUIUtility.IconContent("Toolbar Minus"), EditorStyles.label))
                {
    ...
    // --- Drag & Drop & Injection Methods (Unchanged) ---
    ...
    private void HandleDrop(Rect rect, FolderNode target)
    {
        ...
        {
            if (evt.type == EventType.DragUpdated) { DragAndDrop.visualMode = DragAndDropVisualMode.Move; evt.Use(); }
                ...
                else if (DragAndDrop.GetGenericData("source") != null) ((FolderNode)DragAndDrop.GetGenericData("source")).scriptsInside.Remove(path);
